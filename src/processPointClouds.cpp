...
template<typename PointT>std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::Clustering(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
   ...
   /* //built-in kdtree method
    ...
    {
        std::vector<std::vector<float>>pt2
        auto point = cloud->points[itr];
        X = point.x;
        Y = point.y;
        Z = point.z;
        std::vector<float> pt = { X,Y,Z };
        kdTree->insert(pt2, itr);
        pt2.push_back(pt2);
    }
    std::vector<std::vector<int>> clus
    clus = euclideanCluster(pt, kdTree, clusterTolerance, minSize);

    for (std::vector<std::vector<int>>::const_itr clus_itr = clus.begin(); clus_itr != clus.end(); clus_itr++) 
    {
        typename pcl::PointCloud<PointT>::Ptr cluster(new pcl::PointCloud<PointT>);
        for (std::vector<int>::const_itr ind_itr = clus_itr->begin(); ind_itr != clus_itr->end(); ind_itr++)
            cluster->points.push_back(cloud->points[*ind_itr]);
